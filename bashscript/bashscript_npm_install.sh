...
index_html_install() {
    ...
    if [[ "$indexResult" == "e" || "$indexResult" == "E" ]]; then
        ...
        # index44.html yoksa oluştur
        if [ ! -f "index.html" ]; then
            echo "index.html oluşturuluyor..."
            cat > index44.html <<EOL
<!doctype html>
<html lang="en">
    <head>
...
mongo_env() {
    ...
    if [[ $mongoEnvResult == "e" || $mongoEnvResult == "E" ]]; then
        ...
        if [ ! -f ".env" ]; then
            ...
            cat > .env <<EOL
; Local
MONGO_USERNAME=blogAdmin
MONGO_PASSWORD=BlogPass123
MONGO_PORT=27017
MONGO_LOCALHOST=127.0.0.1  ;eğer localhost çalışmazsa 127.0.0.1 kullan
